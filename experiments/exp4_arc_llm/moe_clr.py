...
class Block(nn.Module):
    def __init__(self,d,h,ffn,d_e,E):
...
class LM(nn.Module):
    def __init__(self,ffn,d=256,h=8,L=6,d_e=512,E=8,max_len=600):
        super().__init__(); self.em=nn.Embedding(VOCAB,d); self.pos=nn.Embedding(max_len,d)
        self.blocks=nn.ModuleList([Block(d,h,ffn,d_e,E) for _ in range(L)])
        self.norm=nn.LayerNorm(d); self.head=nn.Linear(d,VOCAB,bias=False); self.is_moe=(ffn=='moe')
    def forward(self,x):
    ...
    def set_k(self,k):
        if self.is_moe:
            for b in self.blocks: b.ffn.set_k(k)
...
if __name__=='__main__':
    ...
    elif a.cond=='moe_closed': run('moe_closed','moe',steps=a.steps,seq=a.seq,batch=a.batch,mode='closed',k_min=2,E=a.E)
