"""训练 FRSMASH-DenseStateMoE(全强度10×状态) vs vanilla, 比 val ppl + 验状态强度."""
...
def run(name, model, steps=1500, seq=512, batch=32, lr=5e-4, wd=0.01, n_val=3000):
    ...
    for st in range(1,steps+1):
        ...
        with torch.amp.autocast('cuda',dtype=torch.bfloat16):
            o=fwd(model,x); loss=F.cross_entropy(o.reshape(-1,VOCAB),y.reshape(-1),ignore_index=0)
    ...
    vp=vppl()
    extra=''
    if hasattr(model,'slow_cell') and hasattr(model.slow_cell,'diag'):
        sn=model.slow_cell.diag.get('state_norm'); extra=f' state_norm(mean)={sn.mean():.1f}'
    print(f'[{name}] DONE val_ppl={vp:.2f}{extra} ({time.time()-t0:.0f}s)\n',flush=True)
    return vp
...
if __name__=='__main__':
    ...
    a=ap.parse_args()
    if a.cond=='vanilla': run('vanilla',FRSMASHv36(VOCAB,a.d,8,a.L,4).to(DEV),a.steps)
    else: run('dense',FRSMASHDenseStateMoE(VOCAB,a.d,8,a.L,4,n_state_experts=10).to(DEV),a.steps)
