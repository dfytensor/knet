...
## 11. 文章指导 → "先拟合再压缩" 救活 SGR

应用 K-Net 论文转述的"先拟合再压缩 + 闭环阀门"到 SGR-v3 路由：
| 方法 | end frac | val ppl | vs dense |
|------|----------|---------|----------|
| dense | 1.0 | 68.10 | — |
| 硬路由从第0步（旧） | 0.4 | 73.84 | +8.4% |
| **退火 dense→sparse** | 0.4 | 69.96 | +2.7% |
| **闭环阀门（自动）** | 0.77 | 69.06 | +1.4% |

**"先拟合再压缩"把 SGR 的 ppl 灾难(+8.4%)救到 +1.4%(闭环)/+2.7%(退火)，gap 缩 68%**，
变成"1.1-1.5× 更省推理、质量近乎持平"的可部署方案。闭环控制器自找拐点（val 退化即放松）。
仍不能在 ppl 上赢 dense（路由有微小固有代价）——**调度救得了 SGR 的可用性，救不了它在 ppl 上赢 dense。**
...
> **但退化止步于玩具任务**，真实 LM 上让位给标准 cosine。基于秩洞察的架构创新(SGR)**在真实 LM 指标
> 上全面失败**；唯一胜出的是**把 softmax 换成 GLA**（与论文无关的现成部件）。
>
> 一句话：**方向感对，不等式方向错；闭环只在强 gap 任务有用；秩洞察造不出更好的 LLM。**
> 描述性洞察 ≠ 处方性设计。
