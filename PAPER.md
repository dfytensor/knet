...
## 25. Loop 10: FRSMASH State-MoE (10× 状态) — 不提升(负例)

SlowMemory 改成 N=10 专家 + top-1 路由(总状态 10×, 稀疏激活~1×, 仅慢 5%)。~82M d512/L8:
seq512/1500步: vanilla ppl **37.10** vs StateMoE 37.38(+0.8% 噪声); seq1024/800步: 52.87 vs 52.86(完全持平)。**短长上下文均无提升**。
根因: ① SlowMemory 非瓶颈(SSM骨干+GLA recall 是主状态); ② top-1 路由碎片化状态(每专家只整合~1/10 token, 10×容量被 10×碎片抵消); ③此规模 state 非容量受限。
与 loop9 一致: **FRSMASH 均衡设计, 加法失效**(hybrid 冗余/state-MoE 非瓶颈)。要让 10×状态有用得套主状态(SSM骨干)+反碎片化+超长上下文。详见 experiments/exp5_frsmash_hybrid/FRSMASH_STATEMOE_RESULT.md。
